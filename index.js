...
async function fetchCandles(ticker, count = M1_CANDLES_NEEDED) {
  ...
  } catch (e) { console.error("[CANDLES] fetch error:", e); return null; }
}
...
function checkEntryQuality(candles, signal) {
  ...
  if (!ema20 || !atr14) {
    return { pass: false, rejects: ["REJECT_BAD_INDICATOR_DATA"], debug: {} };
  }
  ...
  if (signal === "SHORT" && bearish < 2) rejects.push(`REJECT_NO_PRESSURE (only ${bearish}/3 bearish candles)`);
  if (signal === "LONG"  && bullish < 2) rejects.push(`REJECT_NO_PRESSURE (only ${bullish}/3 bullish candles)`);
  ...
  for (const c of last2) {
    ...
    if (signal === "SHORT") {
      ...
      if (upperWick / body > M1_MAX_WICK_TO_BODY_RATIO) {
        rejects.push(`REJECT_WICK_REJECTION (upper wick ${upperWick.toFixed(2)} > ${M1_MAX_WICK_TO_BODY_RATIO}x body ${body.toFixed(2)})`);
        break;
      }
    }
  }
  ...
  if (signal === "SHORT") {
    if (!(parseFloat(curr.close) < parseFloat(prev.low))) rejects.push("REJECT_NO_BREAK_TRIGGER");
  } else {
    if (!(parseFloat(curr.close) > parseFloat(prev.high))) rejects.push("REJECT_NO_BREAK_TRIGGER");
  }
...
app.get("/status", (_req, res) => {
  ...
  for (const [ticker, trade] of activeTrades.entries()) {
    trades.push({
      ticker, signal: trade.signal, status: trade.status,
  ...
  res.json({
    ...
    inSession: isInSession(), qualityFilterActive: !!TWELVEDATA_KEY,
    openTrades: trades.filter(t => t.status === "OPEN"),
    allTrades: trades, cooldowns,
  });
});
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    const { ticker, signal, entry, atr, tf, strategy } = parsed;
    ...
    if (!signal) return res.status(400).json({ ok: false, error: "Missing or invalid signal. Expected LONG/SHORT or BUY/SELL." });
    if (!ticker) return res.status(400).json({ ok: false, error: "Missing ticker/symbol." });
    if (!Number.isFinite(entry)) return res.status(400).json({ ok: false, error: "Missing valid entry/price in webhook payload." });
    ...
    if (candles) {
      ...
      if (!quality.pass) {
        ...
        return res.status(200).json({ ok: true, accepted: false, reason: "Rejected by quality filters", rejects: quality.rejects, debug: quality.debug });
      }
    } else {
    ...
    if (decision.reason === "REVERSE" && decision.reverse) {
      ...
      await sendTelegram(
        `🔄 <b>REVERSED — ${ticker}</b>\n` +
        `Closed: ${prev.signal} @ <code>${prev.entry}</code>\n\n` +
        formatEntry(ticker, signal, newLevels, atr, tf, strategy)
      );
      console.log(`[REVERSE] ${prev.signal} → ${signal} ${ticker} @ ${entry}`);
      return res.json({ ok: true, accepted: true, status: "reversed_and_opened", ticker, signal, ...newLevels });
    }
    ...
    return res.json({ ok: true, accepted: true, status: "opened", signal, ticker, ...levels });
