...
// News window config (minutes 25–35 of each hour)
...
async function getHTFTrend(ticker) {
  ...
  console.log(
    `[HTF] M5 EMA50=${r(ema50Curr)} (prev=${r(ema50Prev)}, ${rising ? "rosnące↑" : falling ? "spadające↓" : "płaskie→"}), ` +
...
app.get("/status", (_req, res) => {
  const trades = [];
  for (const [ticker, trade] of activeTrades.entries()) {
    trades.push({ ticker, signal: trade.signal, status: trade.status,
  ...
  res.json({ botStatus: "running", time: new Date().toUTCString(),
    inSession: isInSession(), newsTime: isNewsTime(),
    qualityFilterActive: !!TWELVEDATA_KEY,
    openTrades: trades.filter(t => t.status === "OPEN"), allTrades: trades, cooldowns });
});
  ...
  try {
    ...
    if (candles && candles.length >= 15) {
      ...
      if (realAtr && realAtr >= MIN_ATR && realAtr <= MAX_ATR) {
        console.log(`[ATR] Live ATR: ${r(realAtr)} (payload był: ${r(atr)})`);
        atr = realAtr;
      } else {
    ...
    console.log(`[ENTRY] ${signal} ${ticker} @ ${entry} ATR=${r(atr)} HTF=${htfTrend ?? "N/A"}`);
    return res.status(200).json({ ok: true, accepted: true, status: "opened", signal, ticker, htfTrend, ...levels });
