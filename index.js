...
const M1_EMA_MAX_DISTANCE_MULT  = 1.5;
...
function getWarsawNowParts() {
  const parts = new Intl.DateTimeFormat("en-GB", {
    timeZone: "Europe/Warsaw", weekday: "short", hour: "numeric", minute: "numeric", hour12: false,
...
async function fetchCandles(ticker, count = M1_CANDLES_NEEDED, interval = "1min") {
  ...
  try {
    ...
    const res  = await fetch(`https://api.twelvedata.com/time_series?symbol=${sym}&interval=${interval}&outputsize=${count}&apikey=${TWELVEDATA_KEY}`);
    const json = await res.json();
    if (!json.values || !Array.isArray(json.values)) { console.warn(`[CANDLES] TwelveData (${interval}): ${json.message ?? "brak danych"}`); return null; }
...
async function fetchPrice(ticker) {
  if (!TWELVEDATA_KEY) return null;
  try {
    const res  = await fetch(`https://api.twelvedata.com/price?symbol=${encodeURIComponent(toTwelveSymbol(ticker))}&apikey=${TWELVEDATA_KEY}`);
    const json = await res.json();
...
async function getHTFTrend(ticker) {
  ...
  if (!candles || candles.length < HTF_EMA_PERIOD + 1) { console.warn(`[HTF] Za mało świec M5 (${candles?.length ?? 0}) — pomijam HTF filtr`); return null; }
  const closes = candles.map(c => parseFloat(c.close));
  const price  = closes[closes.length - 1];
  const ema50Curr = calcEMA(closes, HTF_EMA_PERIOD);
  const ema50Prev = calcEMA(closes.slice(0, -1), HTF_EMA_PERIOD);
  if (!ema50Curr || !ema50Prev) { console.warn("[HTF] Nie można obliczyć EMA50 M5 — pomijam HTF filtr"); return null; }
  const rising = ema50Curr > ema50Prev, falling = ema50Curr < ema50Prev;
  let trend = "NEUTRAL";
  if (price > ema50Curr && rising)  trend = "LONG";
  if (price < ema50Curr && falling) trend = "SHORT";
  console.log(`[HTF] M5 EMA50=${r(ema50Curr)} (prev=${r(ema50Prev)}, ${rising ? "rosnące↑" : falling ? "spadające↓" : "płaskie→"}), cena=${r(price)}, trend=${trend}`);
  return trend;
}
...
function buildLevels(signal, entry, atr) {
  const slD = Math.min(Math.max(atr * SL_MULTIPLIER, MIN_SL_DIST), MAX_SL_DIST);
  ...
  return {
    ...
    slDist: r(slD), tp1Dist: r(slD * TP1_MULTIPLIER), tp2Dist: r(slD * TP2_MULTIPLIER), tp3Dist: r(slD * TP3_MULTIPLIER),
  };
}
...
function canOpen(ticker, signal, entry, atr) {
  ...
  if (lastAt !== undefined) {
    ...
    if (elapsed < COOLDOWN_MS) { const rem = Math.ceil((COOLDOWN_MS - elapsed) / 60000); return { ok: false, reason: `Cooldown — jeszcze ${rem} min` }; }
  }
  if (!isInSession()) {
    const { weekday, hour, minute } = getWarsawNowParts();
    return { ok: false, reason: `Poza sesją (teraz ${weekday} ${hour}:${String(minute).padStart(2,"0")} Warszawa | Pn–Czw 06:00–22:00, Pt 06:00–${FRIDAY_END}:00)` };
  }
  if (isNewsTime()) { const { minute } = getWarsawNowParts(); return { ok: false, reason: `REJECT_NEWS_TIME_WINDOW (minuta ${minute}, okno :${NEWS_WINDOW_START}–:${NEWS_WINDOW_END})` }; }
  const levels = buildLevels(signal, entry, atr);
  const risk = Math.abs(entry - levels.sl), reward = Math.abs(levels.tp1 - entry);
  const realRR = risk > 0 ? r(reward / risk) : 0;
  if (realRR < RR_MIN) return { ok: false, reason: `REJECT_RR_TOO_LOW (RR=${realRR} < min ${RR_MIN}, risk=${r(risk)}, reward=${r(reward)})` };
...
function closeTrade(ticker, closePrice, reason) {
  ...
  if (!trade) return;
  const pnl = r(trade.signal === "LONG" ? closePrice - trade.entry : trade.entry - closePrice);
...
async function sendTelegram(text) {
  ...
  try {
    ...
    console.log("[TG] Wiadomość wysłana OK"); return { ok: true };
...
function checkEntryQuality(candles, signal) {
  ...
  const last2 = candles.slice(-2), last3 = candles.slice(-3), last5 = candles.slice(-5);
  const ema20 = calcEMA(closes, M1_EMA_PERIOD), atr14 = calcATR(candles);
  ...
  if (emaDistAtr > M1_EMA_MAX_DISTANCE_MULT)
    rejects.push(`REJECT_TOO_FAR_FROM_EMA (${emaDistAtr.toFixed(2)}x ATR > max ${M1_EMA_MAX_DISTANCE_MULT}x)`);

  const recentHigh = Math.max(...highs.slice(-10)), recentLow = Math.min(...lows.slice(-10));
  ...
  const last3H = last3.map(c => parseFloat(c.high)), last3L = last3.map(c => parseFloat(c.low));
  ...
  return {
    ...
    debug: { ema20: r(ema20), atr14: r(atr14), emaDistAtr: `${emaDistAtr.toFixed(2)}x`, moveDist: r(moveDist), avgBody: r(avgBody), choppyPairs, pressureBars, currentPrice: r(currentPrice) },
  };
}
...
async function checkTrades() {
  for (const [ticker, trade] of activeTrades.entries()) {
    ...
    if (isLong ? price <= trade.sl : price >= trade.sl) {
      ...
      const pnl = r(trade.signal === "LONG" ? price - trade.entry : trade.entry - price);
      closeTrade(ticker, price, "SL"); lastSignalAt.set(ticker, Date.now());
      await sendTelegram(`🛑 <b>SL HIT — ${ticker} #${trade.id}</b>\nPozycja: ${trade.signal} @ <code>${trade.entry}</code>\nSL (<i>${slLabel}</i>): <code>${r(trade.sl)}</code>  |  Cena: <code>${r(price)}</code>\n💵 <b>P&L: <code>${pnl >= 0 ? "+" : ""}${pnl} pkt</code></b>\nOtwarto: ${trade.openedAt}\nZamknięto: ${new Date().toUTCString()}`);
    }
  }
}
...
function startPriceMonitor() {
  ...
  setInterval(async () => {
    ...
    const now = Date.now();
    if (hasOpen) await checkTrades().catch(console.error);
    else if (now - lastIdleCheck > 5*60*1000) { lastIdleCheck = now; await checkTrades().catch(console.error); }
  }, 30_000);
}
...
function parseWebhookPayload(body) {
  ...
  return {
    raw,
    signal:   normalizeSignal(raw.signal ?? raw.side ?? raw.action ?? raw.order_action ?? raw.direction),
    ticker:   normalizeTicker(raw.ticker ?? raw.symbol ?? raw.instrument ?? raw.market ?? "XAUUSD"),
    entry:    n(raw.entry ?? raw.price ?? raw.close ?? raw.last ?? raw.trigger_price, null),
    atr:      n(raw.atr ?? raw.atr14 ?? raw.atr_14, DEFAULT_ATR),
    tf:       String(raw.tf ?? raw.timeframe ?? raw.interval ?? "M1"),
    strategy: String(raw.strategy ?? raw.system ?? "TV_WEBHOOK"),
  };
}
...
app.get("/health", (_req, res) => res.status(200).json({
  ok: true, status: "ok", uptimeSec: Math.floor(process.uptime()),
  activeTrades: [...activeTrades.values()].filter(t => t.status === "OPEN").length,
  time: new Date().toISOString(), inSession: isInSession(), newsTime: isNewsTime(),
}));

app.get("/status", (_req, res) => {
  const trades = [];
  for (const [, trade] of activeTrades.entries())
    trades.push({ id: trade.id, ticker: trade.ticker, signal: trade.signal, status: trade.status,
      entry: trade.entry, sl: trade.sl, slOriginal: trade.slOriginal,
      tp1: trade.tp1, tp2: trade.tp2, tp3: trade.tp3, tp1Hit: trade.tp1Hit, tp2Hit: trade.tp2Hit,
      openedAt: trade.openedAt, closedAt: trade.closedAt ?? null,
      closePrice: trade.closePrice ?? null, closeReason: trade.closeReason ?? null, pnlPts: trade.pnlPts ?? null });
  const cooldowns = [];
  for (const [ticker, ts] of lastSignalAt.entries())
    cooldowns.push({ ticker, cooldownMinRemaining: Math.max(0, Math.ceil((COOLDOWN_MS - (Date.now()-ts)) / 60000)) });
  res.json({
    ...
    cooldowns,
    config: { accountBalance: ACCOUNT_BALANCE, riskPercent: RISK_PERCENT, slMultiplier: SL_MULTIPLIER,
              rrMin: RR_MIN, cooldownMin: COOLDOWN_MS/60000,
...
app.get("/test-signal", async (_req, res) => {
  try {
    ...
    res.json({ ok: true, telegram: tgResult, entry, atr: r(atr), levels });
...
app.get("/admin/close", async (req, res) => {
  ...
  closeTrade(ticker, price, reason); lastSignalAt.set(ticker, Date.now());
  const pnl = trade.pnlPts, pnlStr = pnl >= 0 ? `+${pnl}` : `${pnl}`;
  await sendTelegram(`🔧 <b>RĘCZNE ZAMKNIĘCIE — ${ticker} #${trade.id}</b>\nPozycja: ${trade.signal} @ <code>${trade.entry}</code>\nZamknięto @ <code>${r(price)}</code>  (<i>${reason}</i>)\n💵 <b>P&L: <code>${pnlStr} pkt</code></b>\n<i>${new Date().toUTCString()}</i>`);
...
app.get("/admin/reset-cooldown", (req, res) => {
  const ticker = String(req.query.ticker ?? "XAUUSD").toUpperCase();
  if (lastSignalAt.has(ticker)) { lastSignalAt.delete(ticker); console.log(`[ADMIN] Cooldown zresetowany dla ${ticker}`); return res.json({ ok: true, message: `Cooldown zresetowany dla ${ticker}` }); }
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (candles && candles.length >= 15) {
      ...
      if (realAtr && realAtr >= MIN_ATR && realAtr <= MAX_ATR) { console.log(`[ATR] Live ATR: ${r(realAtr)} (payload: ${r(atr)})`); atr = realAtr; }
      else console.log(`[ATR] Live ATR ${r(realAtr)} poza zakresem — zostawiam: ${r(atr)}`);
    }
    ...
    } else console.warn(`[QUALITY] Brak świec dla ${ticker} — proceduję bez filtrów jakości`);
    ...
    if (!decision.ok) { console.log(`[REJECT] ${ticker} ${signal}: ${decision.reason}`); return res.status(200).json({ ok: true, accepted: false, reason: decision.reason }); }
    ...
    const tradeId = activeTrades.get(ticker).id;
    console.log(`[ENTRY] #${tradeId} ${signal} ${ticker} @ ${entry} ATR=${r(atr)} HTF=${htfTrend ?? "N/A"}`);
    return res.status(200).json({ ok: true, accepted: true, status: "opened", id: tradeId, signal, ticker, htfTrend, ...levels });
