...
function buildLevels(signal, entry, atr) {
  ...
  return {
    ...
    rr1:     r(TP1_MULTIPLIER), rr2: r(TP2_MULTIPLIER), rr3: r(TP3_MULTIPLIER),
    slDist:  r(slD),
...
function registerTrade(ticker, signal, levels) {
  activeTrades.set(ticker, {
    ticker, signal,
    entry: levels.entry, sl: levels.sl, tp1: levels.tp1, tp2: levels.tp2, tp3: levels.tp3,
    status: "OPEN", openedAt: new Date().toUTCString(),
    tp1Hit: false, tp2Hit: false, closedAt: null, closePrice: null, closeReason: null,
  });
...
function closeTrade(ticker, closePrice, reason) {
  ...
  trade.closePrice = r(closePrice); trade.closeReason = reason;
}
...
function checkEntryQuality(candles, signal) {
  ...
  if (signal === "SHORT" && !(last3H[2] < last3H[1]))
    rejects.push(`REJECT_NO_LOWER_HIGHS (ostatni szczyt ${last3H[2].toFixed(2)} nie poniżej ${last3H[1].toFixed(2)})`);
  if (signal === "LONG"  && !(last3L[2] > last3L[1]))
    rejects.push(`REJECT_NO_HIGHER_LOWS (ostatni dołek ${last3L[2].toFixed(2)} nie powyżej ${last3L[1].toFixed(2)})`);
  ...
  for (const c of last2) {
    ...
    if (signal === "SHORT") {
      ...
      if (lowerWick / body > M1_MAX_WICK_TO_BODY_RATIO) { rejects.push(`REJECT_WICK_REJECTION (dolny knot ${lowerWick.toFixed(2)} > ${M1_MAX_WICK_TO_BODY_RATIO}x korpus ${body.toFixed(2)})`); break; }
    } else {
      const upperWick = h - Math.max(o, cl);
      if (upperWick / body > M1_MAX_WICK_TO_BODY_RATIO) { rejects.push(`REJECT_WICK_REJECTION (górny knot ${upperWick.toFixed(2)} > ${M1_MAX_WICK_TO_BODY_RATIO}x korpus ${body.toFixed(2)})`); break; }
    }
  }
  ...
  for (let i = 1; i < last5.length; i++) {
    ...
    if (totalRange > 0 && overlap / totalRange > M1_CHOP_OVERLAP_THRESHOLD) choppyPairs++;
  }
  ...
  return { pass: rejects.length === 0, rejects, debug: { ema20: r(ema20), atr14: r(atr14), emaDist: `${emaDist.toFixed(3)}%`, moveDist: r(moveDist), avgBody: r(avgBody), choppyPairs, pressureBars, currentPrice: r(currentPrice) } };
}
...
app.get("/health", (_req, res) => res.status(200).json({ ok: true, status: "ok", uptimeSec: Math.floor(process.uptime()), activeTrades: [...activeTrades.values()].filter(t => t.status === "OPEN").length, time: new Date().toISOString(), inSession: isInSession() }));

app.get("/status", (_req, res) => {
  const trades = [...activeTrades.values()].map(t => ({ ticker: t.ticker, signal: t.signal, status: t.status, entry: t.entry, sl: t.sl, tp1: t.tp1, tp2: t.tp2, tp3: t.tp3, tp1Hit: t.tp1Hit, tp2Hit: t.tp2Hit, openedAt: t.openedAt, closedAt: t.closedAt ?? null, closePrice: t.closePrice ?? null, closeReason: t.closeReason ?? null }));
  const cooldowns = [...lastSignalAt.entries()].map(([ticker, ts]) => ({ ticker, cooldownMinRemaining: Math.max(0, Math.ceil((COOLDOWN_MS - (Date.now() - ts)) / 60000)) }));
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (candles && candles.length >= 15) {
      ...
      if (realAtr && realAtr >= MIN_ATR && realAtr <= MAX_ATR) { console.log(`[ATR] Live ATR: ${r(realAtr)}`); atr = realAtr; }
    }
    ...
    if (!decision.ok) { console.log(`[REJECT] ${decision.reason}`); return res.status(200).json({ ok: true, accepted: false, reason: decision.reason }); }
    ...
    console.log(`[ENTRY] ${signal} ${ticker} @ ${entry} ATR=${r(atr)}`);
